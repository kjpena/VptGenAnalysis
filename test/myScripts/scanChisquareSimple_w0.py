...
tagList=[ #('central',                'p_{T,min}^{2}=0.8, Pythia8 CUEP8M2T4'),
...
#tagList+=[ ('central_tune%d'%tuneId, 'p_{T,min}^{2}=0.8, Pythia8 Tune %d'%tuneId) for tuneId in xrange(24,29) ]
tagList+=[ ('ptsqmin4_tune%d'%tuneId,'p_{T,min}^{2}=4.0, Pythia8 Tune %d'%tuneId) for tuneId in xrange(24,29) ]
...
for tags,legend in tagList: #Loop over tags; e.g. ptsqmin4_tune2
    ...
    for name,title in plotList: #Loop over plots d##-x01-y04
        ...
        for wgtIdx,muR,muF in wgtList: #Loop over weights
            ...
            h=getPlotFrom('%s/ZJ_%s/ZJ_%s.w%d.root'%(rootDir,tags,tags,wgtIdx),name,'mc') #Get MC histogram
        ...
        #print name,wgtIdxmin,muRmin,muFmin,chi2min,ndof
        print chi2min
        ...
        #if reducedMuSpace :
        #    c.SaveAs('~/www/chi2_plots_new/ReducedMuSpace/ZJ_%s/%s.png'%(tags,name))
        #    c.SaveAs('~/www/chi2_plots_new/ReducedMuSpace/ZJ_%s/%s.pdf'%(tags,name))
        #    c.SaveAs('~/www/chi2_plots_new/ReducedMuSpace/ZJ_%s/%s.root'%(tags,name))
